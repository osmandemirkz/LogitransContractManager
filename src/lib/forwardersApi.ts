...
/** Per-language-count stamp/signature position offset (px relative to default position) */
export interface AssetPosition {
  x: number;
  y: number;
}
export interface PositionConfig {
  stamp: { '1': AssetPosition; '2': AssetPosition; '3': AssetPosition };
  sig:   { '1': AssetPosition; '2': AssetPosition; '3': AssetPosition };
}
export const DEFAULT_POSITION_CONFIG: PositionConfig = {
  stamp: { '1': { x: 0, y: 0 }, '2': { x: 0, y: 0 }, '3': { x: 0, y: 0 } },
  sig:   { '1': { x: 0, y: 0 }, '2': { x: 0, y: 0 }, '3': { x: 0, y: 0 } },
};
...
export interface Forwarder {
  ...
  /** Per-language stamp/signature position offsets */
...
function fromRow(row: any): Forwarder {
  return {
    ...
    positionConfig: row.position_config || DEFAULT_POSITION_CONFIG,
...
function toRow(f: Partial<Forwarder>): Record<string, any> {
  ...
  if (f.positionConfig !== undefined) row.position_config = f.positionConfig;
