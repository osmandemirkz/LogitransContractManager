...
import {
  Draft, saveDraft, cleanupExpiredDrafts
} from '@/lib/draftsApi';
...
const Index: React.FC = () => {
  ...
  const autoSaveDraft = useCallback(async () => {
    ...
    // Never auto-save over a signed contract — would overwrite 'signed' status with 'draft'
  ...
  }, [pageMode, currentDraftId, isContractSigned, currentUser?.id, contract]);
  ...
  const handleLoadDraft = (draft: Draft) => {
    ...
    setIsContractSigned(draft.status === 'signed');
    setPageMode('editor');
  };
  ...
  const handleContractSigned = async (pdfUrl: string) => {
    // ExportOptions already updated Supabase directly; just mark local state so
    // auto-save doesn't overwrite the 'signed' status with 'draft'.
    setIsContractSigned(true);
    console.log('[handleContractSigned] Contract signed, auto-save disabled for this session. pdfUrl:', pdfUrl);
  };
      ...
      >
        ...
        <div className="flex-1 overflow-x-auto">
          <ExportOptions
            ...
            managerDriveFolderId={(loadedManagers.find(m => m.id === (formData.managerId || currentUser?.id)) as any)?.drive_folder_id || ''}
          />
        </div>
