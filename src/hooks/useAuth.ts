...
export interface StoredManager {
  ...
  /** Google Drive folder ID — PDFs uploaded by this manager will also be saved here */
...
export async function saveManager(
  manager: { name: string; email: string; companyId: string; password?: string; extra_sheets?: ExtraSheet[]; drive_folder_id?: string }
  ...
  const newMgr: any = {
    ...
    drive_folder_id: manager.drive_folder_id || '',
  };
  const { data, error } = await supabase
    .from('logitrans_managers')
    .insert(newMgr)
