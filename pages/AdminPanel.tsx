...
import {
  ...
  uploadForwarderAsset, removeForwarderAsset
...
const AdminPanel: React.FC = () => {
  ...
  const [newMgr, setNewMgr] = useState({ name: '', email: '', companyId: 'logitrans_uz', password: '', extra_sheets: [] as ExtraSheet[] });
  ...
  useEffect(() => {
    if (tab === 'storage') { loadStorageStats(); fetchLiveRates(); loadMainSheets(); }
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: 'hsl(215 15% 48%)' }}>"Manager Ata" ile müşteriyi manager'a bağlayın</p>
              </div>
              <div className="flex items-center gap-2">
                ...
                {allDrafts.map(draft => {
                  ...
                  return (
                    <div key={draft.id} className="rounded-xl p-4" style={{ background: isAssigning ? 'hsl(220 28% 20%)' : isSharing ? 'hsl(215 28% 20%)' : 'hsl(215 28% 17%)', border: `1px solid ${isAssigning ? 'hsl(220 60% 40% / 0.5)' : isSharing ? 'hsl(38 92% 50% / 0.4)' : 'hsl(215 22% 25%)'}` }}>
                      <div className="flex items-center gap-3">
                        <div className="flex items-center justify-center w-9 h-9 rounded-lg flex-shrink-0" style={{ background: 'hsl(215 25% 24%)' }}><FileText size={16} style={{ color: 'hsl(38 92% 50%)' }} /></div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="text-sm font-semibold" style={{ color: 'hsl(210 20% 88%)' }}>{draft.contractNumber || 'Черновик'}</p>
                            {draft.status === 'signed' && <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: 'hsl(142 50% 20%)', color: 'hsl(142 70% 60%)' }}>✓ Подписан</span>}
                            {assignedMgr && (
