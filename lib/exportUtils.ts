...
import { getForwarder } from '@/lib/forwardersApi';
...
function buildExpeditorCell(
  ...
  const stampHtml = includeStamp && fwd.stampUrl
    ? `<img src="${fwd.stampUrl}" style="height:${stmpH}px;width:${stmpW}px;object-fit:contain;opacity:0.85;margin-left:16px;" />`
    : includeStamp
    ? `<div style="display:inline-block;border:3px solid #1a3a6b;border-radius:50%;width:${stmpW}px;height:${stmpH}px;text-align:center;font-size:6pt;font-weight:bold;color:#1a3a6b;padding:8px;box-sizing:border-box;">LOGITRANS<br/>SEAL<br/>М.П.</div>`
    : '';

  const sigHtml = includeSignature && fwd.signatureUrl
    ? `<img src="${fwd.signatureUrl}" style="height:${sigH}px;width:auto;object-fit:contain;position:absolute;bottom:-4px;left:70px;" />`
    : includeSignature
    ? `<span style="font-family:cursive;font-size:11pt;color:#1a3a6b;margin-left:8px;">${dirName}</span>`
    : '';

  const sigBlock = `<div style="margin-top:16px;display:flex;align-items:flex-end;gap:16px;">
    <div style="position:relative;">${labelDir} ______________________ ${dirName}${sigHtml}</div>
    ${stampHtml}
  </div>`;
...
export async function generateContractHTML(data: ContractFormData): Promise<string> {
  ...
  // 11.1 Expeditor row
  const exp11Row = `<tr>
    ${showRu ? buildExpeditorCell('ru', fwd, company, data.includeStamp, data.includeSignature, stampScale, signatureScale) : ''}
    ${showEn ? buildExpeditorCell('en', fwd, company, data.includeStamp, data.includeSignature, stampScale, signatureScale) : ''}
    ${showTr ? buildExpeditorCell('tr', fwd, company, data.includeStamp, data.includeSignature, stampScale, signatureScale) : ''}
  </tr>`;
